...
float* MovingImage::GetColors()
{
    return _color;
}

bool& MovingImage::GetShouldDraw()
{
    return _shouldDraw;
}
...
void MovingImage::Update(sf::RenderTarget& target)
{
    ...
    m_sprite.setColor(sf::Color(_color[0] * 255.f, _color[1] * 255.f , _color[2] * 255.f));
    if (!isStanding && !isPause) {
        m_sprite.setPosition({m_sprite.getPosition().x + _xSpeed, m_sprite.getPosition().y + _ySpeed});
    }
}
