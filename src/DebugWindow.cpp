...
void DebugWindow::setDebugData(const DebugData& newData) {
    data = newData;
}

void DebugWindow::Render(std::shared_ptr<MovingImage> img, std::shared_ptr<Text> text) {
    ...
    ImGui::SliderFloat("Speed", &(data.speedValue), 0.0f, 100.0f); // Slider for float value
    ImGui::SliderFloat("Size", &(data.sizeValue), 0.1f, 5.f); // Slider for float value

    // Color picker
    ImGui::ColorEdit3("Color Picker", data.color); // Color picker

    ImGui::Text("Selected Color: (%.2f, %.2f, %.2f)", data.color[0], data.color[1], data.color[2]);
